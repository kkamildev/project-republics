...
public class AlignedTextGroup : TextGroup
{
    ...
    public AlignedTextGroup(Text[] texts, Vector2 margin, Aligment aligment = Aligment.VERTICAL) : base(texts)
    {
        _aligment = aligment;
        _margin = margin;
    ...
    public override Vector2 MainPosition {
        ...
        {
            _mainPosition = value;
            if(_aligment == Aligment.VERTICAL)
            {
            ...
            if(_aligment == Aligment.HORIZONTAL)
            {
                for(int i = 0; i < _texts.Length; i++)
                {
